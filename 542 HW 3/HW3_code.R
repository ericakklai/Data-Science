load("~/Desktop/STAT 542/HW3/hw3Data.Rdata")
library(MASS)
#Problem 1
...
mysd[2,which(mysd[2,]==0)] = min(mysd[2,which(mysd[2,]!=0)])
...
mysd.pc[2,which(mysd.pc[2,]==0)] = min(mysd.pc[2,which(mysd.pc[2,]!=0)])
...
bestsize=9
prune_tree=prune.tree(tree.data,best=bestsize, method = "misclass")
summary(prune_tree)
...
test.err.tree
print(length(tree.data$frame$var))
print(length(prune_tree$frame$var))
